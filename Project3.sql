...
-- Q1. Find the titles and ratings of movies released in 2010 along with their genres.
SELECT g.title, r.rating, g.genre, r.year FROM general_info g INNER JOIN ratings r ON g.id=r.id WHERE year=2010;

-- OR

SELECT r.title, r.rating, g.genre FROM ratings r JOIN general_info g ON r.title = g.title WHERE r.year =2010;

-- Q2. Retrieve a list of actors for each movie along with their ratings. Include only movies with ratings greater than 8.
SELECT p.title, p.actors, r.rating FROM people p JOIN ratings r ON p.title= r.title WHERE r.rating>8;

-- OR 

SELECT p.title, p.actors, r.rating FROM people p JOIN ratings r ON p.id= r.id WHERE r.rating>8  ORDER BY rating DESC;

-- Q3. Find the total revenue for movies directed by 'Christopher Nolan’.
SELECT p.director, SUM(f.revenue) AS total_revenue FROM people p JOIN financial_data f ON p.title = f.title WHERE p.director = "christopher nolan" GROUP BY p.director;
 
 
-- Q4. Display the titles, genres, and directors for movies released in 2015. Include movies that do not have ratings.
SELECT general_info.title, general_info.genre, people.director, ratings.year, ratings.rating FROM general_info LEFT JOIN people ON general_info.title = people.title LEFT JOIN ratings ON general_info.title = ratings.title WHERE ratings.year = 2015; 

-- Q5. List the titles, directors, and revenue for movies that have ratings. Include movies even if they don't have revenue information.
SELECT r.title, p.director, f.revenue FROM ratings r LEFT JOIN people p ON r.title= p.title LEFT JOIN financial_data f ON r.title= f.title;
