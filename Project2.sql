...
-- Q1. How many minutes does the longest movie last?
SELECT MAX(runtime) FROM financial_data;

-- or 

SELECT MAX(runtime) AS longest_film FROM financial_data;

-- or

SELECT title,runtime FROM financial_data WHERE runtime = (SELECT MAX(runtime) FROM financial_data);


-- Q2. and what about the shortest?
SELECT MIN(runtime) FROM financial_data;

-- or

SELECT title,runtime FROM financial_data WHERE runtime = (SELECT MIN(runtime) FROM financial_data);


-- Q3. Is there just one movie that lasts 0 minutes?
SELECT * from financial_data WHERE runtime = 0;

-- OR

SELECT title,runtime FROM financial_data WHERE runtime = 0;



-- Let’s repeat the procedure but subsetting just the movies released on 2012--
-- Q4. How many minutes lasts the longest movie?
SELECT MAX(runtime) FROM financial_data WHERE year=2012;

-- OR

SELECT title,runtime FROM financial_data WHERE year = 2012 ORDER BY runtime DESC limit 2; 


-- Q5. and what about the shortest?
...
-- Q6. Which is the average time for the movies released that year?
SELECT AVG(runtime) from financial_data WHERE year= 2012;

-- Q7.  Which are the highest and lowest rating values?

 SELECT MAX(rating), MIN(rating) FROM ratings WHERE year = 2012;

-- Q8. And if we round them?
SELECT ROUND(MAX(rating)),ROUND(MIN(rating)) FROM ratings;

-- Q9. Which is year of the movie that has the max number of votes?
SELECT * FROM ratings WHERE votes= (SELECT MAX(votes)FROM ratings);

-- OR

SELECT title, year, votes FROM ratings WHERE votes= (SELECT MAX(votes)FROM ratings);



-- Q10. Which is the total number of votes that we have?
SELECT SUM(votes) AS total_votes FROM ratings;

-- OR

SELECT SUM(votes) FROM ratings;

-- Q11. Calculate the frequency table for the year variable in ratings table
SELECT year, COUNT(*) FROM ratings GROUP BY year;


-- Q12. Order the table in descending order 
SELECT year, COUNT(*) FROM ratings GROUP BY year ORDER BY year DESC;


-- Q13.  Get the total number of votes per year
SELECT year, SUM(votes) FROM ratings GROUP BY year;

-- Q14. Which is the director with more movies? (TIP Calculate freq table from director column in people table)
SELECT director,COUNT(*) FROM people GROUP BY director ORDER BY count(*) DESC;

-- OR 

SELECT director, COUNT(*) AS director_total_count FROM people GROUP BY director ORDER BY director_total_count DESC limit 1;

-- Q15. Get the title of the movies directed by this director
SELECT title FROM people WHERE director = "christopher nolan";

-- Q16.From these movies, which is the one that lasts long? And the shortest?
SELECT MAX(runtime), MIN(runtime)FROM financial_data WHERE director = 'christopher nolan';

-- OR

SELECT title FROM financial_data WHERE director = "christoper nolan" ORDER BY runtime DESC limit 1;

-- OR 

SELECT title FROM financial_data WHERE director = "christoper nolan" ORDER BY runtime ASC limit 1;
